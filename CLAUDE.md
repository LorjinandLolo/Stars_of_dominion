...
- Narrative: `lib/narrative/chronicle.ts` records what actually happened (`ChronicleEvent`, append-only, real actor in `actorIds`, public knowledge ceiling in `attribution`). Call sites use `chronicle.record(world, draft)`; the game loop flushes the buffer once per cycle AFTER the snapshot commits. Design and phase plan: `docs/narrative-system/README.md`.
